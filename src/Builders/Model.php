...
class Model{
    ...
    public function __construct(
        ...
        // array_column($connection->getSchemaBuilder()->getTables(), 'name');
        // $connection->getDoctrineSchemaManager()->listDatabases();

        $this->indexes = Schema::getColumnListing($this->params['table_name']);
        $this->fields = [];
        ...
        $this->modelRestDir = base_path().'/app/Models/_rest';
    }
    ...
    public function build(){
        $this->getModelFields();
        $template = file_get_contents(base_path().'/vendor/olivion/rest-gen/src/Templates/Models/Rest.php');
        ...
        if(!file_exists($this->modelRestDir) && !is_dir($this->modelRestDir)){
            mkdir($this->modelRestDir);
        }
        file_put_contents(base_path().'/app/Models/_rest/'.$this->buildParams['ParamModel'].'.php',$template);

        $main_path = base_path().'/app/Models/';
        if($this->buildParams['ParamService'])
        {
            if(!file_exists(base_path().'/app/Models/'.$this->buildParams['ParamService']) && !is_dir(base_path().'/app/Models/'.$this->buildParams['ParamService'])){
                mkdir(base_path().'/app/Models/'.$this->buildParams['ParamService']);
            }

            $main_path = base_path().'/app/Models/'.$this->buildParams['ParamService'].'/';
        }
        

        if(!file_exists($main_path.$this->buildParams['ParamModel'].'.php',)){
            $template = file_get_contents(base_path().'/vendor/olivion/rest-gen/src/Templates/Models/Base.php');
            foreach($this->buildParams as $param => $value){
                $template = str_replace($param,$value,$template);
            }
            file_put_contents($main_path.$this->buildParams['ParamModel'].'.php',$template);
        }
    ...
    private function getMutatorTemplate($type,$attr){
        if(!file_exists(base_path().'/vendor/olivion/rest-gen/src/Mutators/'.$type.'.mutator')) return '';
        $template = file_get_contents(base_path().'/vendor/olivion/rest-gen/src/Mutators/'.$type.'.mutator');
    ...
    private function generateProperties(){
        $output = [];
        foreach($this->fields as $field){
            $template = file_get_contents(base_path().'/vendor/olivion/rest-gen/src/Templates/Models/ModelProperty.template');
